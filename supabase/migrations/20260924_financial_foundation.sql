-- ==============================================================================
-- AGRA JAYA POS — FINANCIAL FOUNDATION MIGRATION #01
-- Target Database: PostgreSQL (Supabase)
-- Mode: Idempotent, Non-Destructive, Strict Double-Entry Accounting
-- ==============================================================================
...
-- SEED GENERIC PAYMENT ACCOUNTS (SAFE, NO FABRICATED BANK NUMBERS)
DO $$
DECLARE
    v_kas_id BIGINT;
    v_bank_id BIGINT;
...
-- ------------------------------------------------------------------------------
-- 7. JOURNAL POSTING RPC ENGINE (ATOMIC BALANCE VALIDATOR & IDEMPOTENT)
-- ------------------------------------------------------------------------------
...
BEGIN
    ...
    -- 2. Idempotency Check (Kembalikan jika sudah diposting sebelumnya)
    ...
    -- 4. Verifikasi Keseimbangan Akuntansi Mutlak (Double-Entry Balance)
...
-- ------------------------------------------------------------------------------
-- 8. REVERSAL RPC ENGINE (IMMUTABLE VOID REVERSAL)
-- ------------------------------------------------------------------------------
...
BEGIN
    ...
    -- 5. Copy Lines dengan Menukar Debit dan Kredit (Strict Swap)
...
-- Helper Function untuk cek role pengguna yang sedang login
...
-- POLICIES: journal_entries & journal_lines
DROP POLICY IF EXISTS p_journal_owner ON public.journal_entries;
CREATE POLICY p_journal_owner ON public.journal_entries FOR ALL TO authenticated
USING (public.get_current_user_role() = 'owner')
WITH CHECK (public.get_current_user_role() = 'owner');

DROP POLICY IF EXISTS p_journal_read_admin ON public.journal_entries;
CREATE POLICY p_journal_read_admin ON public.journal_entries FOR SELECT TO authenticated
USING (public.get_current_user_role() = 'admin');

DROP POLICY IF EXISTS p_journal_lines_owner ON public.journal_lines;
CREATE POLICY p_journal_lines_owner ON public.journal_lines FOR ALL TO authenticated
USING (public.get_current_user_role() = 'owner')
WITH CHECK (public.get_current_user_role() = 'owner');

DROP POLICY IF EXISTS p_journal_lines_read_admin ON public.journal_lines;
CREATE POLICY p_journal_lines_read_admin ON public.journal_lines FOR SELECT TO authenticated
USING (public.get_current_user_role() = 'admin');
